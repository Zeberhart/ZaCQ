...
class TaskExtractor:
    ...
    def __init__(self):
        ...
        with open("coderegex.txt", "r") as c_file:
            self.regexes = [line.strip() for line in c_file if line.strip()]
        with open("domain.txt", "r") as d_file:
            self.domain_terms = [line.strip() for line in d_file if line.strip()]
#         with open("verbs.txt", "r") as v_file:
#             self.verbs = [line.strip() for line in v_file if line.strip()]
        with open("filtered_verbs.txt", "r") as v_file:
            self.filtered_verbs = [line.strip() for line in v_file if line.strip()]
        with open("generics.txt", "r") as g_file:
            self.generic_terms = set([line.strip() for line in g_file if line.strip()])
#         self.verb_lemmas = set([getLemma(verb, "VERB")[0] for verb in self.verbs])
        self.filtered_verb_lemmas = set([getLemma(verb, "VERB")[0] for verb in self.filtered_verbs])
#         self.verbs_vbz = set()
#         self.verbs_vbg = set()
#         for verb in self.verbs:
#             vbzs = getInflection(verb, "VBZ")
#             self.verbs_vbz.add(vbzs[0])
#             vbgs = getInflection(verb, "VBG")
#             self.verbs_vbg.add(vbgs[0])
